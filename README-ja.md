...
#### シニアソフトウェアエンジニア (2024/01 - , 4 days/week)

- フロントエンドの CI/CD のパフォーマンス改善やデプロイ自動化の推進
  - 12 分以上かかっていた CI の実行時間を 5 分以上削減した
  - テストケースが増えても実行時間が増加しないような仕組みを整備した
- SaaS のテナント管理 (作成、更新、削除) サービスの実装と運用
  - Go と MySQL を利用したジョブキューシステムを実装した
  - k8s クラスタへサーバーをデプロイし、Grafana を利用したモニタリング基盤を整備した
- Web フロントエンドの監視基盤の整備
...
##### [サイボウズ Office のフロントエンド刷新プロジェクト](https://blog.cybozu.io/entry/2023/09/25/080000) (2023/09 - 2023/12)
